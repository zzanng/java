...
public class ArrayExer2 {
	...
	public static void main(String[] args) {
		...
		//실습2 1~100까지의 정수 중에서 홀수만 배열에 담아 출력 
		...
		System.out.printf("%10d %10f %10s",99999,3.141592,"hello");  //자리수 맞춰 출력 
		
		//실습3 아래 데이터로 2차원 배열을 만들어서 출력 
		// 이름 국어 영어 수학 
		// 홍길동 100 90 80
		// 강감찬  90 80 70
		// 이순신  80 70 60
		...
		//실습4 개인별 과목별 총점과 평균을 실습3에 추가하여 출력
		...
		for(int i =0; i<nameArrLeng2; i++) {
			...
			for(int j =0; j<scoreArrLeng2; j++) {
				...
				pAgg[i][0] += scoreArr2[i][j]; //i += 2  => i=i+2
				sAgg[j][0] += scoreArr2[i][j];
				System.out.printf("%10d",scoreArr2[i][j]);			
	
			}
			pAgg[i][1] = pAgg[i][0]/scoreArrLeng2;
			System.out.printf("%10d %10d",pAgg[i][0],pAgg[i][1]);		
		}
		for (int i=0; i<scoreArrLeng2; i++) {
			sAgg[i][1] = sAgg[i][0]/scoreArrLeng2;
		}
